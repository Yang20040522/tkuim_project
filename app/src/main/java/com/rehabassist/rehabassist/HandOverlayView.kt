...
class HandOverlayView @JvmOverloads constructor(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        var wx = if (rotation == 90 || rotation == 270) wrist.y() * width else wrist.x() * width
        // ✨ 同學的手腕特效也加上鏡像魔法
        if (isFrontCamera) wx = width - wx
        ...
        if (showStickGuide && landmarks.size >= 18) {
            ...
            var ix = if (rotation == 90 || rotation == 270) indexMcp.y() * width else indexMcp.x() * width
            var px = if (rotation == 90 || rotation == 270) pinkyMcp.y() * width else pinkyMcp.x() * width
        ...
        if (showPinchGuide && landmarks.size >= 9) {
            ...
            var tx = if (rotation == 90 || rotation == 270) thumbTip.y() * width else thumbTip.x() * width
            var idx = if (rotation == 90 || rotation == 270) indexPip.y() * width else indexPip.x() * width
