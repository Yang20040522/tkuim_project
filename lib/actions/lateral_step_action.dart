...
class LateralStepAction
  ...
  TrainingLegRole get role {
    if (_trainedLegIsLeft == null ||
        _supportLegIsLeft == null) {
  ...
  RehabFeedback update(BodyFrame frame) {
    ...
    final hipWidth =
        (movingHip.dx - supportHip.dx).abs();

    // 防止髖寬太小導致門檻異常。
    final safeHipWidth =
        math.max(hipWidth, 0.08);
    ...
    if (role ==
      ...
      if (_supportHipXHistory.length >
          _supportHistoryFrames) {
        _supportHipXHistory.removeAt(0);
      }

      if (_supportHipXHistory.length >= 6) {
        final minX =
            _supportHipXHistory.reduce(math.min);

        final maxX =
            _supportHipXHistory.reduce(math.max);

        final sway =
            maxX - minX;

        if (sway >
            _supportSwayTolerance) {
          return RehabFeedback(
            prompt: _throttled(
              '支撐腳晃動比較大,請站穩再繼續',
            ),
    ...
    switch (_state) {
      ...
      case _StepState.waiting:
        if (stepDelta >= triggerStepDelta) {
          ...
          // 人還沒開始跨時，
          // 緩慢更新站姿基準，吸收骨架小抖動。
          if ((currentHorizontalFootDistance -
                      baseline)
                  .abs() <
              0.015) {
            _baselineHorizontalFootDistance =
  ...
  bool _upgrade() {
    ...
    if (difficulty ==
        RehabDifficulty.easy) {
      difficulty =
          RehabDifficulty.medium;
